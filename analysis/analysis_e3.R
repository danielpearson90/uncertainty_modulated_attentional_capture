...
source(here::here("preprocessing", "preprocessing_e3.R"))
...
single_hv_lv <- t.test(
  single_df %>% filter(trial_label == "HV") %>% pull(mean_gaze_on_dist),
  single_df %>% filter(trial_label == "LV") %>% pull(mean_gaze_on_dist),
  paired = TRUE
)
...
single_lv_nv <- t.test(
  single_df %>% filter(trial_label == "LV") %>% pull(mean_gaze_on_dist),
  single_df %>% filter(trial_label == "NV") %>% pull(mean_gaze_on_dist),
  paired = TRUE
)
...
wilcox_chce_HV_NV <- wilcox.test(x = choice_df_averaged %>% filter(trial_label == "HV vs NV") %>% pull(mean_choice), mu = 0)
...
wilcox_chce_LV_NV <- wilcox.test(x = choice_df_averaged %>% filter(trial_label == "LV vs NV") %>% pull(mean_choice), mu = 0)
...
estimation_posthoc_bonf <- pairs(estimation_posthoc, adj = "bonferroni")
