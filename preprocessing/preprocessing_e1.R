...
data_files <- fs::dir_ls(here("data", "e1", "raw_data", "BehavData"),
  glob = "*.csv"
)
choice_files <- fs::dir_ls(here("data", "e1", "raw_data", "ChoiceData"),
  glob = "*.csv"
)
estimation_files <- fs::dir_ls(here("data", "e1", "raw_data", "EstimationData"),
  glob = "*.csv"
)
...
quality_df <- unprocessed_data_df %>%
  group_by(subnum) %>%
  summarise(
    trials_completed = max(fullTrial),
    mean_fixation_prop = mean(fixationPropGoodSamples),
    mean_trial_prop = mean(trialPropGoodSamples),
    num_timeouts = sum(timeout),
    num_soft_timeouts = sum(softTimeoutTrial),
    num_bad_trialprop_trials = sum(trialPropGoodSamples < .25)
  )
