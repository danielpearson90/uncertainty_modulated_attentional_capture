...
if realVersion
    ...
    iti = 0.01;            % 1.2
    feedbackDuration = [0.01, 0.01, 0.01];       % [0.7, 1.5, 1.5]  FB duration: Practice, first block of expt phase, later in expt phase
    
    yellowFixationDuration = 0.01;     % Duration for which fixation cross turns yellow to indicate trial about to start
    blankScreenAfterFixationPause = 0.01;        % UPDATED 14/01/16 IN LINE WITH FAILING ET AL. PAPER
    
    initialPause = 0.01;   % 2 ***
    breakDuration = 0.01;  % 15 ***
    
    % SPECTRUM STUFF
    requiredTargetDwellTime = 0.01;     % Time that target must be fixated for trial to be successful
    omissionTimeLimit = 0;          % Dwell time on distractor that means this will be an omission trial
    requiredFixationDwellTime = 0.01;       % Time that fixation cross must be fixated for trial to begin
    
    pracTrials = 8;
    numExptBlocksPhase = [1, 1, 2];   % Phase 1 = practice, Phase 2 = single distractor only, Phase 3 = mixed single and double distractor
    
    singlePerBlock = [0, 1, 1];
    doublePerBlock = [0, 0, 1];
